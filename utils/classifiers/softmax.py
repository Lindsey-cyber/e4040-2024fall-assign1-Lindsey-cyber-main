...
def softmax(x):
    ...
    h = x / np.sum(x, axis = 1, keepdims = True)
...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(N):
        for j in range(K):
            P[i][j] = np.sum(X[i,:] * W[:,j])
            
    P = softmax(P)
    loss = np.sum(cross_entropy(G, P)) / N + 0.5 * reg * np.sum(W ** 2)
  
    for i in range(D):
        for j in range(K):
            dW[i, j] = np.sum(X.T[i, :] * (P - G)[:, j]) / N
    dW += reg * W
...
def cross_entropy(p, q):
    ...
    h = - np.sum(p * np.log(q + 1e-15), axis = 1)
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    P = softmax(P)
    G = onehot(y, K)
    loss = np.sum(cross_entropy(G, P)) / N + 0.5 * reg * np.linalg.norm(W, ord = 2)
    dW = X.T @ (P - G) / N + reg * W
